 ...
 */
public class CrossSmoothCheckBox extends SmoothCheckBox {
    ...
    private droidninja.filepicker.views.SmoothCheckBox.OnCheckedChangeListener mListener;

    public CrossSmoothCheckBox(Context context) {
        this(context, null);
    }

    public CrossSmoothCheckBox(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public CrossSmoothCheckBox(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        init(attrs);
    }

    @TargetApi(Build.VERSION_CODES.LOLLIPOP)
    public CrossSmoothCheckBox(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
    ...
    private void init(AttributeSet attrs) {
        ...
        TypedArray ta = getContext().obtainStyledAttributes(attrs, droidninja.filepicker.R.styleable.SmoothCheckBox);
        int tickColor = ta.getColor(droidninja.filepicker.R.styleable.SmoothCheckBox_color_tick, COLOR_TICK);
        mAnimDuration = ta.getInt(droidninja.filepicker.R.styleable.SmoothCheckBox_duration, DEF_ANIM_DURATION);
        mFloorColor = ta.getColor(droidninja.filepicker.R.styleable.SmoothCheckBox_color_unchecked_stroke, COLOR_FLOOR_UNCHECKED);
        mCheckedColor = ta.getColor(droidninja.filepicker.R.styleable.SmoothCheckBox_color_checked, COLOR_CHECKED);
        mUnCheckedColor = ta.getColor(droidninja.filepicker.R.styleable.SmoothCheckBox_color_unchecked, COLOR_UNCHECKED);
        mStrokeWidth = ta.getDimensionPixelSize(droidninja.filepicker.R.styleable.SmoothCheckBox_stroke_width,
                dp2px(getContext(), 0));
        ta.recycle();
    ...
    public void setChecked(boolean checked) {
        ...
        invalidate();
        if (mListener != null) {
            mListener.onCheckedChanged(CrossSmoothCheckBox.this, mChecked);
        }
    }
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        mTickPoints[0].y = Math.round((float) getMeasuredHeight() / 30 * 10);
        mTickPoints[1].x = Math.round((float) getMeasuredWidth() / 30 * 22);
        mTickPoints[1].y = Math.round((float) getMeasuredHeight() / 30 * 20);
        mTickPoints[2].x = Math.round((float) getMeasuredWidth() / 30 * 7);
        mTickPoints[2].y = Math.round((float) getMeasuredHeight() / 30 * 20);
        mTickPoints[3].x = Math.round((float) getMeasuredWidth() / 30 * 22);
        mTickPoints[3].y = Math.round((float) getMeasuredHeight() / 30 * 10);

        mLeftLineDistance = (float) Math.sqrt(Math.pow(mTickPoints[1].x - mTickPoints[0].x, 2) +
                Math.pow(mTickPoints[1].y - mTickPoints[0].y, 2));
        mRightLineDistance = (float) Math.sqrt(Math.pow(mTickPoints[3].x - mTickPoints[2].x, 2) +
                Math.pow(mTickPoints[3].y - mTickPoints[2].y, 2));
    ...
    private void drawTickPath(Canvas canvas) {
        ...
        if (mDrewDistance < mLeftLineDistance) {
            ...
            if (mDrewDistance < mLeftLineDistance + mRightLineDistance) {
                ...
                float stopX = mTickPoints[2].x + (mTickPoints[3].x - mTickPoints[2].x) * (mDrewDistance - mLeftLineDistance) / mRightLineDistance;
                float stopY = mTickPoints[2].y + (mTickPoints[3].y - mTickPoints[2].y) * (mDrewDistance - mLeftLineDistance) / mRightLineDistance;
    ...
    public void setOnCheckedChangeListener(droidninja.filepicker.views.SmoothCheckBox.OnCheckedChangeListener l) {
        this.mListener = l;
    }

    public interface OnCheckedChangeListener {
        void onCheckedChanged(droidninja.filepicker.views.SmoothCheckBox checkBox, boolean isChecked);
    }
